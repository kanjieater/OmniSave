...
def test_romm_device_id_stable_after_username_rename(client, conn, monkeypatch):
    ...
    pinned = _db.get_user_config(conn, admin_uid, "romm_source_id")
    assert pinned == f"romm:{admin_uid}", f"expected romm_source_id to be pinned, got {pinned!r}"
    ...
    device_id = _romm_vsc.get_user_romm_device_id(conn, admin_uid)
    assert device_id == f"romm:{admin_uid}", (
        f"device ID shifted after rename: got {device_id!r}, want romm:{admin_uid}"
    )
